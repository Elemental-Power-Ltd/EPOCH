...
TEST_F(EpochSimulationRun, CommonTaskData) {
	...
	*/
	...
	EXPECT_FLOAT_EQ(result.comparison.carbon_balance_scope_2, 71933.156f);
	EXPECT_FLOAT_EQ(result.comparison.cost_balance, 2116.125f);
	EXPECT_FLOAT_EQ(result.comparison.payback_horizon_years, 16.482113f);
...
TEST_F(EpochSimulationRun, FullTaskData) {
	...
	*/
	...
	EXPECT_FLOAT_EQ(result.comparison.carbon_balance_scope_2, -11579.254f);
	EXPECT_FLOAT_EQ(result.comparison.cost_balance, 174634.28f);
	EXPECT_FLOAT_EQ(result.comparison.payback_horizon_years, 5.2370772f);
