...
TEST_F(FundingTest, ReceivesBoilerUpgradeFunding) {
	...
	config.use_boiler_upgrade_scheme = true;
	auto capex_with_funding = calculate_capex_with_discounts(siteData, config, scenario);
	...
	config.use_boiler_upgrade_scheme = false;
	auto capex_without_funding = calculate_capex_with_discounts(siteData, config, scenario);
...
TEST_F(FundingTest, BaselineWithoutBoiler) {
	...
	config.use_boiler_upgrade_scheme = true;
	auto capex = calculate_capex_with_discounts(sd, config, scenario);
...
TEST_F(FundingTest, ScenarioKeepsBoiler) {
	...
	config.use_boiler_upgrade_scheme = true;
	auto capex = calculate_capex_with_discounts(siteData, config, scenario);
...
TEST_F(FundingTest, GeneralGrant) {
	...
	config.general_grant_funding = 50000.0f;
	auto capex_with_funding = calculate_capex_with_discounts(siteData, config, scenario);

	EXPECT_LT(capex_with_funding.total_capex, capex_without_funding.total_capex);


	config.general_grant_funding = 1000000000.0f;
	auto capex_with_billion_grant = calculate_capex_with_discounts(siteData, config, scenario);
