...
public:
    ...
    void initialise(float initialESUM) {
        ...
        initialise_TS_ESS_before_grid_discharge(initialESUM);
    ...
    void runTimesteps(const year_TS& ESUM) {
        ...
        for (int timestep = 1; timestep < mTimesteps; timestep++) {
            ...
            calculate_TS_ESS_available_discharge_power(timestep);
            ...
            calculate_TS_ESS_available_charge_power(timestep);
            ...
            calculate_TS_ESS_before_grid_discharge(ESUM[timestep], timestep);
    ...
    void initialise_TS_ESS_available_discharge_power() {
    ...
    void initialise_TS_ESS_available_charge_power() {
        // calculate kW power from energy kWh
        float ESS_start_SoC_power = mStartSoC * mCapacity / mTimestep_hours; 
        float charge_potential = (mCapacity - ESS_start_SoC_power) / mRTE;

        mAvailableChargePower[0] = std::min(charge_potential, mChargePower);;
        return;
    }

    void initialise_TS_ESS_before_grid_discharge(float initialESUM) {
        // calculate kW power from energy kWh 
        if (initialESUM > 0) {
    ...
    void initialise_TS_ESS_Rgen_only_charge(float initialESUM) {
        // calculate kW power from energy kWh 
        if (initialESUM < 0) {
    ...
    void initialise_TS_ESS_discharge() {
        if (mDischargeMode == 1) {
    ...
    void initialise_TS_ESS_resulting_SoC() {
        // calculate kW power from energy kWh
    ...
    void calculate_TS_ESS_available_discharge_power(int timestep) {
    ...
    void calculate_TS_ESS_available_charge_power(int timestep) {
        // calculate kW power from energy kWh
        float prev_resulting_SoC = mResultingSoC[timestep - 1];
        //energy to power
        float ESS_prev_SoC_power = prev_resulting_SoC / mTimestep_hours;
    ...
    void calculate_TS_ESS_before_grid_discharge(float Esum_TS, int timestep) {

        float ESS_dis_TS = mAvailableDischargePower[timestep];
        ...
        mBeforeGridDischarge[timestep] = TS_before_grid_discharge;
    }

    void calculate_TS_ESS_Rgen_only_charge(float Esum_TS, int timestep) {
        //EC4 = IF(Esum!C4<0,MIN(-ESum!C4,ESS!CC4),0)
        if (Esum_TS < 0) {
            mRgenOnlyCharge[timestep] = std::min(-Esum_TS, mAvailableChargePower[timestep]);
        } else {
            mRgenOnlyCharge[timestep] = 0.0f;
        }
    }

    void set_TS_ESS_discharge(int timestep) {
        if (mDischargeMode == 1) {
    ...
    void set_TS_ESS_charge(int timestep) {
        if (mChargeMode == 1) {
    ...
    void calculate_TS_ESS_resulting_SoC(int timestep) {
        // calculate kW power from energy kWh
        float TS_ESS_end_SoC_energy = mResultingSoC[timestep - 1] + 
        ...
        mResultingSoC[timestep] = TS_ESS_end_SoC_energy;
    }
