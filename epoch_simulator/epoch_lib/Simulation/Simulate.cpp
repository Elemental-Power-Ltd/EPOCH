...
FullSimulationResult Simulator::simulateScenarioFull(const HistoricalData& historicalData, const TaskData& taskData, SimulationType simulationType) const {
	...
	if (taskData.CAPEX_limit*1000 < myCost.get_project_CAPEX())
	{
		...
		fullSimulationResult.total_annualised_cost = FLT_MAX; //Effectively a NULL value that GA can very quickly discount solution
		fullSimulationResult.project_CAPEX = myCost.get_project_CAPEX();
		fullSimulationResult.scenario_cost_balance = -FLT_MAX;
		fullSimulationResult.payback_horizon_years = FLT_MAX;
		fullSimulationResult.scenario_carbon_balance = -FLT_MAX;
