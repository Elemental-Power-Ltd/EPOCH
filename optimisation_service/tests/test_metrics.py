...
class TestCalculateCarbonCost:
    ...
    def test_null_and_negtative_carbon_balance_scope_1(self, carbon_balance_scope_1: float) -> None:
    ...
    def test_null_and_negtative_capex(self, capex: float) -> None:
...
class TestCalculateCalculatePaybackHorizon:
    ...
    def test_negative_operating_balance(self) -> None:
        res = calculate_payback_horizon(capex=10, operating_balance=-10)
        assert res < 0

    def test_null_operating_balance(self) -> None:
        res = calculate_payback_horizon(capex=10, operating_balance=0)
        assert res < 0

    def test_null_vs_negative_operating_balance(self) -> None:
