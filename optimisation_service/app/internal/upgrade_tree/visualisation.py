...
def draw_upgrade_tree(
    ...
    if ax is None:
        _, ax = plt.subplots()
    pos: dict[str, tuple[float, float]] = nx.get_node_attributes(G, "pos")  # type: ignore[no-untyped-call]
    labels: dict[str, str] = nx.get_node_attributes(G, "label")  # type: ignore[no-untyped-call]
    ...
    nx.draw_networkx_edge_labels(
        ...
        edge_labels={
            k: f"£{v:.2f}"
            for k, v in nx.get_edge_attributes(G, "operating_cost").items()  # type: ignore[no-untyped-call]
        },
        font_size=8,
    )
    operating_costs: dict[str, float] = nx.get_node_attributes(G, "operating_cost")  # type: ignore[no-untyped-call]
