...
def run_headless(
    ...
    assert pathlib.Path(
        full_path_to_exe
    ).exists(), f"Could not find an EPOCH executable at {full_path_to_exe.resolve(strict=False)}"
    ...
    input_dir = pathlib.Path(input_dir).resolve(strict=False)
    output_dir = pathlib.Path(output_dir).resolve(strict=False)
    config_dir = pathlib.Path(config_dir).resolve(strict=False)
...
class PyTaskData(TaskData):  # pyright: ignore
    """
    ...
    _INTEGER_KEYS: Final[set[str]] = {"ESS_charge_mode", "ESS_discharge_mode", "target_max_concurrency"}
...
class PySimulationResult:  # pyright: ignore
    """
