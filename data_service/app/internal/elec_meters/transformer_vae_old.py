...
class VAE(nn.Module):  # noqa: D101
    def __init__(
        ...
        # use ReLU here -- cheap default, and all inputs should be nonnegative anyway

        # Fully connected layer to produce mean and log variance for the latent space
        self.encoder_fc = nn.Sequential(
            nn.Linear(hidden_dim * 2, hidden_dim * 2), nn.ReLU(), nn.Linear(hidden_dim * 2, latent_dim * 2), nn.ReLU()
        )
        # Combine LSTM output with non-temporal features
        # input here will be concatenated output of self.lstm_encoder() and self.fc_non_temporal
        ...
        self.lstm_decoder = nn.LSTM(latent_dim + hidden_dim + 1, hidden_dim + 1, num_layers, batch_first=True)
        ...
        # TODO (2025-06-11, JSM) The intermediate dimensions in these nets (and the encoder/decoder nets) could be parameterised
        self.fc_init_hidden = nn.Sequential(nn.Linear(latent_dim + hidden_dim, hidden_dim), nn.LeakyReLU())
        # self.fc_init_hidden = nn.Sequential(nn.Linear(latent_dim + hidden_dim, latent_dim + hidden_dim), nn.LeakyReLU(),
        #                                     nn.Linear(latent_dim + hidden_dim, hidden_dim), nn.LeakyReLU())
        self.fc_init_cell = nn.Sequential(nn.Linear(latent_dim + hidden_dim, hidden_dim), nn.LeakyReLU())
        # self.fc_init_cell = nn.Sequential(nn.Linear(latent_dim + hidden_dim, latent_dim + hidden_dim), nn.LeakyReLU(),
        #                                   nn.Linear(latent_dim + hidden_dim, hidden_dim), nn.LeakyReLU())
        self.fc_init_seq = nn.Sequential(nn.Linear(latent_dim + hidden_dim, hidden_dim), nn.LeakyReLU())
        # self.fc_init_seq = nn.Sequential(nn.Linear(latent_dim + hidden_dim, latent_dim + hidden_dim), nn.LeakyReLU(),
        #                                  nn.Linear(latent_dim + hidden_dim, hidden_dim), nn.LeakyReLU())
        ...
        self.decoder_fc = nn.Sequential(nn.Linear(hidden_dim + 1, input_dim), nn.LeakyReLU())  # no final activation
        # self.decoder_fc = nn.Sequential(nn.Linear(hidden_dim+1, hidden_dim), nn.LeakyReLU(),
        #                                 nn.Linear(hidden_dim, input_dim), nn.LeakyReLU()) # no final activation
    ...
    def decode_old(
        ...
        non_temporal_input = torch.cat([aggregate, start_date, end_date], dim=1)  # Shape: (batch_size * n_days, 2*date_dim+1)
        processed_non_temporal = self.fc_non_temporal(non_temporal_input)  # Shape: (batch_size * n_days, hidden_dim)

        # Combine z with processed non-temporal features to form the context vector
        context_vector = torch.cat([z, processed_non_temporal], dim=1)
        # Shape: (batch_size * n_days, latent_dim + hidden_dim)
        ...
        # Initial input to the LSTM (tiled context vector, projected to hidden_dim)
        tiled_context_vector = context_vector.unsqueeze(1).repeat(1, seq_len, 1)
        # Shape: (batch_size * n_days, seq_len, latent_dim + hidden_dim)
        lstm_input = self.fc_init_seq(tiled_context_vector).to(z.device)
        # Shape: (batch_size * n_days, seq_len, hidden_dim)
        ...
        lstm_input = torch.cat(
            ...
            dim=2,
        )  # Shape: (batch_size * n_days, seq_len, hidden_dim + 1)
        # Concatenate zeros to corresponding dimensions of h_0, c_0 to ensure matching dimensions
        h_0 = torch.cat([h_0, torch.zeros(1, batch_size * n_days, 1).to(z.device)], dim=2)
        c_0 = torch.cat([c_0, torch.zeros(1, batch_size * n_days, 1).to(z.device)], dim=2)
        ...
        decoded_seq, _ = self.lstm_decoder(lstm_input, (h_0, c_0))  # Shape: (batch_size * n_days, seq_len, hidden_dim + 1)
    ...
    def decode(
        ...
        # non_temporal_input = torch.cat([aggregate, start_date, end_date], dim=1)  # Shape: (batch_size * n_days, 2*date_dim+1)
        # processed_non_temporal = self.fc_non_temporal(non_temporal_input)  # Shape: (batch_size * n_days, hidden_dim)
        processed_non_temporal = torch.zeros(batch_size * n_days, self.hidden_dim).to(z.device)

        # No context vector; just input z
        context_vector = z  # Shape: (batch_size * n_days, latent_dim)

        # h0 and c0 should be projected versions of the latent
        # input to the lstm should be tiled latent

        # Initialize the LSTM hidden state and cell state using the context vector
        h_0 = self.proj_init_hidden(context_vector).unsqueeze(0)  # Shape: (1, batch_size * n_days, hidden_dim)
        c_0 = self.proj_init_cell(context_vector).unsqueeze(0)  # Shape: (1, batch_size * n_days, hidden_dim)

        lstm_input = context_vector.unsqueeze(1).repeat(1, seq_len, 1)
        # Shape: (batch_size * n_days, seq_len, latent_dim)
        ...
        # DON'T NEED MATCHING DIMENSIONS FOR lstm_input, h0, c0 # Concatenate zeros to corresponding dimensions of h_0, c_0 to ensure matching dimensions
        # h_0 = torch.cat([h_0, torch.zeros(1, batch_size*n_days, 1).to(z.device)], dim=2)
        # c_0 = torch.cat([c_0, torch.zeros(1, batch_size*n_days, 1).to(z.device)], dim=2)
...
class TransformerVAE(nn.Module):
    ...
    def forward(
        ...
        hh_reconstructed = vae_output["reconstructed"]

        # if np.isnan(hh_reconstructed.detach()).any():
        #     print(f"are there nans in x input? {np.isnan(x.detach()).any()}")
        #     print(f"are there nans in aggregate input? {np.isnan(aggregate.detach()).any()}")
        #     print(f"are there nans in start_date input? {np.isnan(start_date.detach()).any()}")
        #     print(f"are there nans in end_date input? {np.isnan(end_date.detach()).any()}")
        #     print(f"are there nans in output? {np.isnan(hh_reconstructed.detach()).any()}")
        #     raise ValueError("nans experienced in TransformerVAE, possibly inside self.vae")
        ...
        return {
            ...
            "vae_reconstructed": hh_reconstructed,
            "mu": vae_output["mu"],
