...
class GridSearch(Algorithm):
    ...
    def run(self, problem: Problem, verbose: bool = False) -> Result:
        ...
        df_res = pd.read_csv(
            Path(output_dir, "ExhaustiveResults.csv"),
            encoding="cp1252",
            dtype=np.float32,
            usecols=usecols,
        )
        ...
        fitnesses = df_res[objectives].to_numpy()

        senses = problem.objectives.values()
        str_senses = ["max" if sense == -1 else "min" for sense in senses]
        pareto_efficient = paretoset(fitnesses, str_senses, distinct=not self.keep_degenerate)
        solutions = solutions[pareto_efficient]
        fitnesses = fitnesses[pareto_efficient]
...
def EPOCH(
    ...
    """
    Execute EPOCH grid search on a set of parameters.
    ...
    project_path
        path to EPOCH
    config_dir
        Directory of EPOCH config file
    input_dir
        Directory of EPOCH input files
    config_dir
        Directory to ouput files from EPOCH
    """
