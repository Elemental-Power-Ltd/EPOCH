...
class GeneticAlgorithm(Algorithm):
    ...
    def run(self, problem: Problem, verbose: bool = False) -> Result:
        ...
        for sub_problem in problem.split_objectives():
            ...
            for parameter, value in zip(pi.v_params, sol, strict=False):
                pi.TaskData[parameter] = value
            simresult = PySimulationResult(pi.sim.simulate_scenario(pi.TaskData))
            fitnesses.append([simresult[objective] for objective in objectives])

            exec_time += res.exec_time
            n_evals += res.algorithm.evaluator.n_eval

        fitnesses_arr, solutions_arr = np.asarray(fitnesses), np.asarray(solutions)

        fitnesses_arr, non_degen_idx = np.unique(fitnesses_arr, axis=0, return_index=True)
        solutions_arr = solutions_arr[non_degen_idx]

        return Result(
            solutions=solutions_arr,
            fitnesses=fitnesses_arr,
            exec_time=exec_time,
            n_evals=n_evals,
        )
...
class ProblemInstance(ElementwiseProblem):
    ...
    def __init__(self, problem: Problem) -> None:
        ...
        self.v_params, self.lb, ub, self.step = (
            [],
            np.array([]),
            np.array([]),
            np.array([]),
...
def comp_by_cv_and_fitness(pop: Any, P: npt.NDArray, **kwargs: Any) -> npt.NDArray[np.integer]:
    ...
    for i in range(P.shape[0]):
        ...
        if len(feasible) > 0:
            minimum = np.inf
            best_candidates = [cand for cand in feasible if pop[cand].F <= minimum]
